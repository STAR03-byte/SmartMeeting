...
|说话人标注|基于会议参与人列表为转写片段分配 speaker_name/speaker_user_id|未配置参与人时保留空值|
|LLM 大语言模型|配置 LLM_API_KEY 环境变量；可选配置 Ollama 作为备用后端|未配置或调用失败时自动回退到备用模型或规则生成摘要与任务|
...
|LLM_MODEL|gpt-4o-mini|LLM 主模型名称|
|LLM_FALLBACK_PROVIDER|ollama|LLM 备用后端（ollama/mock/none）|
|OLLAMA_BASE_URL|http://127.0.0.1:11434|Ollama 服务地址|
|OLLAMA_MODEL|llama3.1|Ollama 模型名称|
