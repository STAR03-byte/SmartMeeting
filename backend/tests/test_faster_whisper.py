...
from app.services.ai.faster_whisper_service import FasterWhisperTranscriber
...
def test_faster_whisper_raises_when_model_load_fails():
    ...
    with patch("app.services.faster_whisper_service.importlib.import_module") as mock_import, patch(
        ...
        mock_fw = MagicMock()
        mock_fw.WhisperModel.side_effect = RuntimeError("boom")
        ...
        with pytest.raises(WhisperServiceError, match="Failed to load faster-whisper model"):
            transcriber.preload_model()
